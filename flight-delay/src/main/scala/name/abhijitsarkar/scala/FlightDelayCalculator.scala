...
object FlightDelayCalculator {
  ...
  def averageSink[A](a: A) {
    a match {
      case (uniqueCarrier: String, delayCount: Int, totalDelay: Int) =>
        println(s"Delays for carrier ${uniqueCarrier}: " +
          s"${Try(totalDelay / delayCount).getOrElse(0)} average mins, ${delayCount} delayed flights.")
      case x => println(s"No idea what $x is!")
    }
  }

  import FlightDataDownloader._

  val flightDelayLines: Iterator[String] = io.Source.fromFile(extract(download(url, outfile)), "utf-8").getLines()
  ...
  def stringArrayToFlightEvent(cols: Array[String]) = new FlightEvent(cols(0), cols(1), cols(2), cols(3), cols(4),
    cols(5), cols(6), cols(7), cols(8), cols(9), cols(10), cols(11), cols(12), cols(13), cols(14), cols(15),
    cols(16), cols(17), cols(18), cols(19), cols(20), cols(21), cols(22), cols(23), cols(24), cols(25),
    cols(26), cols(27), cols(28))
    ...
    Flow[FlightDelayRecord]
      ...
      .fold(("", 0, 0)) {
        (x: (String, Int, Int), y: FlightDelayRecord) =>
          val delayCount = x._2 + 1
          val runningTotalDelay = x._3 + Try(y.arrDelayMins.toInt).getOrElse(0)
          (y.uniqueCarrier, delayCount, runningTotalDelay)
      }.mergeSubstreams
}
